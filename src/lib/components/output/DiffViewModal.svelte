...
>
  <svelte:fragment slot="head" let:close>
    ...
    <div class="field has-addons mt-3 mr-3">
      {#if changedOldFiles.length > 0}
        <div class="mr-3 mt-2"><b>{updatedTransactionsCount}</b> işlem değişti</div>
      {/if}
      <p class="control">
        <button
          class="button"
    ...
    <div class="box py-0">
      ...
      {#if changedOldFiles.length === 0}
        <div class="has-text-centered mt-6">
          <strong>Hata!</strong> Bir değişiklik yapılmadı. Toplu düzenleme argümanlarının doğru olduğundan emin olun.
        </div>
      {/if}
    </div>
  </div>
  <svelte:fragment slot="foot" let:close>
    ...
    {#if changedOldFiles.length > 0}
      <button
        ...
        on:click={(e) => dispatch("save", changedNewFiles) && close(e)}>Tümünü Kaydet</button
      >
    {/if}
  </svelte:fragment>
</Modal>